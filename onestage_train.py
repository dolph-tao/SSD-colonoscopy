...
parser.add_argument('--dataset_root', default='G:/datasets/polypdatabase/CVC-ClinicDB/',
                    help='Dataset root directory path')
parser.add_argument('--basenet', default='/origins_CVC_clinic_ssd300_polyp_119999.pth',
...
def train():
    ...
    for iteration in range(args.start_iter, cfg_polyp['max_iter']):
        ...
        if iteration % 10 == 0:
            ...
            print('iter ' + repr(iteration) + ' || Loss: %.4f || Loss_c: %.4f || Loss_l: %.4f ' %
                  (loss.item(), loss_c.item(), loss_l.item()))
            writer.add_scalars('polyp_origins_CVC_clinic/train_origin_2time',{'loss_c': float(loss_c.item()),
                                              'loss_l': float(loss_l.item())}, iteration)



        if iteration != 0 and iteration % 5000 == 0:
            print('Saving state, iter:', iteration)
            torch.save(ssd_net.state_dict(), 'AnatomyDetection/origins_2time_CVC_clinic_ssd300_polyp_' +
                       repr(iteration) + '.pth')
    torch.save(ssd_net.state_dict(), 'AnatomyDetection/origins_2time_CVC_clinic_ssd300_polyp_' +
               repr(iteration) + '.pth')
    writer.close()
