...
from ..box_utils import match, log_sum_exp, jaccard
from data.AnatomyDetectionData import DICT_MUTEX
...
class MultiBoxLoss2(nn.Module):
    ...
    def labelmask(self,labels,conf_mask,num_classes,idx):
        mask = torch.ones(num_classes)
        for i in range(labels.shape[0]):
            value = DICT_MUTEX[labels[i].long().item()]
            mask[torch.tensor(value, dtype=torch.long)+1] = 0
        conf_mask[idx] = mask
    ...
    def forward(self, predictions, targets):
        ...
        """
        use_loss = False
        loc_data, conf_data, priors,simi_data = predictions
        num = loc_data.size(0)
        ...
        for idx in range(num):
            ...
            match(self.threshold, truths, defaults, self.variance, labels,
                  loc_t, conf_t, idx)
            if(use_loss):
                self.labelmask(labels, conf_mask, num_classes, idx)
        if self.use_gpu:
            loc_t = loc_t.cuda(cfg['cuda_device'])
            conf_t = conf_t.cuda(cfg['cuda_device'])
            if(use_loss):
                conf_mask = conf_mask.cuda(cfg['cuda_device'])
        # wrap targets
        ...
        # simi_Loss_l = torch.nn.TripletMarginLoss(margin=3)
        ...
        for idx in num_pos:
            for i in range(start_dix, start_dix+idx-1):
                ...
                icount = 0
                for j in range(i+1, start_dix+idx):
